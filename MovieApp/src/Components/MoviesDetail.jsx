...
function MoviesDetail() {
  ...
  const routeMovieId = Number(movieId);
  const movieForRoute = currentMovie?.id === routeMovieId ? currentMovie : null;
  const creditForRoute =
    movieCredit?.movieId === movieForRoute?.id ? movieCredit.data : null;
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/movie/${movieId}?api_key=${API_KEY}`)
      ...
      .catch((err) => {
        ...
        setError({ movieId, message: "We couldn't find that movie." });
      });
  ...
  useEffect(() => {
    if (!movieForRoute?.id) return;
    let ignore = false;

    fetch(`${BASE_URL}/movie/${movieForRoute.id}/credits?api_key=${API_KEY}`)
      .then((res) => {
        if (!res.ok) throw new Error("Credits not found");
        return res.json();
      })
      .then((data) => {
        if (!ignore) setMovieCredit({ movieId: movieForRoute.id, data });
      })
      .catch((err) => {
  ...
  return (
    <section className="mb-20 relative">
      <div
        style={{
          backgroundImage: `url(https://image.tmdb.org/t/p/original${movieForRoute.backdrop_path})`,
        }}
      ...
      <div className="main dark:text-white flex mx-4 md:mx-20 items-center md:items-start space-x-0 md:space-x-4 absolute-400  rounded-b-2xl border-violet-900/30 dark:shadow-0 shadow-lg border-2   rounded-t-3xl backdrop-blur-2xl blur-2x top-10 relative mb-10  light:bg-white/30  md:flex-row flex-col p-4 md:p-10  ">
        ...
        <div className="md:my-0 md:w-240 w-full max-w-64 md:max-w-none md:mx-0 mx-auto my-6 text-center">
          <img
            src={`https://image.tmdb.org/t/p/w500${movieForRoute.poster_path}`}
            alt={movieForRoute.title || "Poster"}
